...
```bash
pg_dump -h localhost -U netraai sentinelops > backup_pre_phase12_$(date +%Y%m%d).sql
```
...
```bash
psql -h localhost -U netraai sentinelops -c "
...
```bash
# Docker
docker-compose restart netraai-backend

# Or systemd
systemctl restart netraai-backend
```
...
cd frontend
npm run build
docker-compose restart netraai-frontend
```

### Step 12: Monitor Initialization
```bash
# Check backend logs
docker logs -f netraai-backend | grep -i phase12

# Check for errors
docker logs netraai-backend | grep -i error
```
...
psql -h localhost -U netraai sentinelops < backup_pre_phase12_YYYYMMDD.sql

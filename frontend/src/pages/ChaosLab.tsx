...
const WORKFLOW_STEPS = [
  { id: 1, label: "Systems Check",        desc: "Validate baseline KPIs and topology integrity" },
  { id: 2, label: "Failure Injection",    desc: "Inject cascading failure into target services" },
  { id: 3, label: "AI Triage Activation",desc: "Multi-agent reasoning and RCA activation" },
  { id: 4, label: "Blast Radius Mapping", desc: "Propagation analysis across dependency graph" },
  { id: 5, label: "Incident Analysis",   desc: "Deep RCA + cascade chain forensics" },
  { id: 6, label: "Recovery Validation", desc: "Autonomous remediation and SLA restoration" },
];
...
export default function ChaosLab() {
  ...
  const runSimulationProtocol = async () => {
    setDemoMode(true);
    setActiveStep(1);
    ...
    <div className="h-full flex flex-col gap-6">
      <header className="flex items-end justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="page-title" style={{ textShadow: "0 0 20px rgba(234,179,8,0.4)" }}>Chaos Lab</h1>
            <p className="text-[10px] font-mono text-gray-500 uppercase tracking-widest mt-1">Failure orchestration · Autonomous scenario injection · Resilience validation</p>
          </div>
        </div>
        <motion.button
          whileHover={{ scale: 1.03 }}
          whileTap={{ scale: 0.97 }}
          onClick={runSimulationProtocol}
          disabled={demoMode}
        ...
        >
          ...
          {demoMode ? "Simulation Running..." : "Run Simulation Protocol"}
        </motion.button>
      </header>
        ...
        <div className="col-span-5 flex flex-col gap-4 h-full overflow-hidden">
          ...
          <div className="glass-panel rounded-xl border border-yellow-500/20 p-4">
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-xs font-display font-semibold text-white">Simulation Protocol</span>
            </div>
